...
namespace quest
{
	...
	ALUA(pc_mount)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("bu haritada binemezsin."));
			return 0;
		}
	...
	ALUA(pc_get_special_ride_vnum)
	{
		...
		if (pChar)
		{
			...
			LPITEM CostumeMount = pChar->GetWear(WEAR_COSTUME_MOUNT);
#endif
...
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
			if (CostumeMount && UNIQUE_GROUP_SPECIAL_RIDE == CostumeMount->GetSpecialGroup())
			{
				lua_pushnumber(L, CostumeMount->GetVnum());
				lua_pushnumber(L, CostumeMount->GetSocket(0)); // @fixme152 (2->0)
				return 3;
			}
#endif
		}
