...
{
    // Replicate horse StateHorse behavior closely for mounts
    ...
    // Thresholds from horse StateHorse
    ...
    {
        // Run when quite far like the horse does
        if (fDist > START_RUN_DISTANCE)
            m_pkChar->SetNowWalking(false);

        // Use CHARACTER::Follow (intercept/pathing) with horse-like approach window
    ...
    {
        // Near owner: wander occasionally (5-12s), identical spirit to StateHorse
        if (now > m_dwLastActionTime)
        {
            ...
            float fx, fy;
            const float fStep = number(200, 400);
            GetDeltaByDegree(m_pkChar->GetRotation(), fStep, &fx, &fy);

            const int nx = m_pkChar->GetX() + (int)fx;
            const int ny = m_pkChar->GetY() + (int)fy;

            if (!(SECTREE_MANAGER::instance().IsAttackablePosition(m_pkChar->GetMapIndex(), nx, ny)
                && SECTREE_MANAGER::instance().IsAttackablePosition(m_pkChar->GetMapIndex(), m_pkChar->GetX() + (int)(fx/2), m_pkChar->GetY() + (int)(fy/2))))
            {
                return true;
            }

            m_pkChar->SetNowWalking(true);
            if (m_pkChar->Goto(nx, ny))
