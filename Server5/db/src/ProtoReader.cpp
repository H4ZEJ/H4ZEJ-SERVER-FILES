...
int get_Item_SubType_Value(int type_value, std::string inputString)
{
	...
	// If this type has no subtype table, treat it as "no subtype" -> 0
	auto itType = mapItemSubType.find(type_value);
	if (itType == mapItemSubType.end())
		return 0;

	const auto& vecSubtype = itType->second;

	// Explicitly handle empty subtype list
	if (vecSubtype.empty())
		return 0;

	// Name-based lookup (strict match)
