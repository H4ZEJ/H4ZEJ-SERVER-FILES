...
bool CHARACTER::ChangeSex()
{
	...
	if (GetWear(WEAR_COSTUME_BODY) || GetWear(WEAR_COSTUME_HAIR)
		...
		|| GetWear(WEAR_NORMAL_PET)
#endif
#ifdef ENABLE_EFFECT_COSTUME_SYSTEM
		|| GetWear(WEAR_EFFECT_ARMOR)
		|| GetWear(WEAR_EFFECT_WEAPON)
#endif
		)
	{
...
#ifdef ENABLE_BLOCK_ITEMS_ON_EVENT_MAP
	if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
		return;
#endif

#ifdef ENABLE_BLOCK_ITEMS_ON_RONARK_PLAYER
	if (CRonarkPlayer::instance().IsRonarkMap(GetMapIndex()))
		return;
#endif

#ifdef ENABLE_BLOCK_ITEMS_ON_DEATHMATCH_MAP
	if (CDeathMatchPvP::instance().IsDeathMatchMap(GetMapIndex()))
		return;
#endif
...
void CHARACTER::MountUnsummon(LPITEM mountItem)
{
	...
	mountSystem->Unsummon(mobVnum, false, false);
}
...
void CHARACTER::CheckMount()
{
	...
	{
		mountSystem->Summon(mobVnum, mountItem, false);
		// mountSystem->Summon(mobVnum, mountItem); //@duzenleme 詰nlannca binek stnden inmezsin..
	}
}
