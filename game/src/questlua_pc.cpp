...
namespace quest
{
	...
	ALUA(pc_change_sex)
	{
		...
		if (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_BODY) || (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_HAIR))
			...
			|| (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_WEAPON))
#endif
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
			|| (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_MOUNT))
#endif
#ifdef ENABLE_EXTENDED_PET_SYSTEM
			|| (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_NORMAL_PET))
#endif
#ifdef ENABLE_EFFECT_COSTUME_SYSTEM
			|| (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_EFFECT_ARMOR))
			|| (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_EFFECT_WEAPON))
#endif
			)
		{
...
#ifdef ENABLE_NEWSTUFF
			//pc.set_race0(race=[0. Warrior, 1. Ninja, 2. Sura, 3. Shaman])
