...
void CHARACTER::CheckMount()
{
	...
	if (mountItem->FindApplyValue(APPLY_MOUNT) != 0)
		mobVnum = mountItem->FindApplyValue(APPLY_MOUNT);
#endif
	const bool isMounted = GetMountVnum() != 0;
	const int summoned = mountSystem->CountSummoned();

	// 1) ZATEN BÝNÝKSEK: takipçi spawn etme (flicker’ý burada kesiyoruz)
	if (isMounted) {
	 // Ýhtiyaten: warp’tan sonra yanlýþlýkla kalmýþ takipçi varsa temizle
	 if (summoned > 0)
		 mountSystem->Unsummon(mobVnum, false);
	 return;
		UpdatePacket();
	}

	// 2) Binili deðilsek: takipçi yoksa summon et; varsa üstüne bindir.
	if (summoned == 0){
	 mountSystem->Summon(mobVnum, mountItem, false);
		UpdatePacket();
	}
