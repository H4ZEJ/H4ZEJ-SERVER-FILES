...
bool CItem::EquipTo(LPCHARACTER ch, BYTE bWearCell)
{
	...
	{
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
		auto isCostumeMount = ITEM_COSTUME == GetType() && COSTUME_MOUNT == GetSubType();
...
bool CItem::Unequip()
{
    ...
    {
        CMountSystem* mountSystem = m_pOwner->GetMountSystem();
        if (mountSystem)
        {
            int applyVal = FindApplyValue(APPLY_MOUNT);
            if (applyVal > 0)
                mountSystem->Unsummon(static_cast<uint32_t>(applyVal));
        }
    }
#endif
