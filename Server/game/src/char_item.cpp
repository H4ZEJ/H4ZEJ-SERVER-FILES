...
bool CHARACTER::UnequipItem(LPITEM item)
{
    ...
    if (item->GetType() == ITEM_COSTUME && item->GetSubType() == COSTUME_MOUNT)
    {
        ...
        if (mountSystem)
        {
            int applyVal = item->FindApplyValue(APPLY_MOUNT); // sadece APPLY_MOUNT
            if (applyVal > 0)
            {
                uint32_t mobVnum = static_cast<uint32_t>(applyVal);
                mountSystem->Unsummon(mobVnum);
            }
            // apply bulunamadýysa unsummon atlanýr; item yine de sökülür.
        }
    }
#endif
...
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
		if (item->GetType() == ITEM_COSTUME && item->GetSubType() == COSTUME_MOUNT && !IS_MOUNTABLE_ZONE(GetMapIndex(), false))
	{
