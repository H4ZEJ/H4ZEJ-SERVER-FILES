...
bool IS_MOUNTABLE_ZONE(int map_index, bool isHorse)
{
	...
	switch (map_index)
	{
		case 81:  // avenor2_wedding
		case 110: // avenor2_t3
		case 112: // avenor2_duelmap
		case 113: // avenor2_ox
		case 254: // avenor2_kingdomwar
		{
			return false;
		}
	}
...
void CHARACTER::MountVnum(DWORD vnum)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("bu haritada binemezsin."));
		return;
	}
...
void CHARACTER::CheckWears()
{
    ...
    int applyVal = item->FindApplyValue(APPLY_MOUNT);
    uint32_t mobVnum = (applyVal > 0) ? static_cast<uint32_t>(applyVal) : 0;

    if (mobVnum == 0)
        return;

			// If player was already mounted, remount directly; otherwise spawn the mount nearby.
			if (GetMountVnumM())
			{
				// Prepare silently to avoid brief double-spawn and mount immediately
				mountSystem->SummonSilent(mobVnum, GetName());
				mountSystem->Mount(mobVnum);
			}
			else
			{
				// Not mounted before: just summon beside the character (no auto-mount)
				mountSystem->Summon(mobVnum, GetName());
			}
