...
#ifdef ENABLE_UPDATE_PACKET_FIX
...
bool CHARACTER::RemoveAffect(CAffect * pkAff)
{
...
#ifdef ENABLE_UPDATE_PACKET_FIX
	bool IsPvPSkill = false; // Ýlk olarak false olarak baþlatýlýr.

	for (auto it : PvpSkills)
	{
		const CAffect* pkAff = FindAffect(it);
		if (pkAff)
		{
			IsPvPSkill = true;
			break; // Affect bulunduðunda döngüden çýk.
		}
	}
	...
	if (IsPvPSkill)
		...
		UpdatePacket();
#else
	if (AFFECT_REVIVE_INVISIBLE != pkAff->dwType && AFFECT_MOUNT != pkAff->dwType)// @fixme329
		ComputePoints();
	else  // @fixme110
		UpdatePacket();
#endif
